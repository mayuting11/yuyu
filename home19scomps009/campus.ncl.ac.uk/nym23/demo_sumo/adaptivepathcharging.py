...
#plot exit node
for i in exitnodeid:
    pyplot.scatter (node_dic[i][0],node_dic[i][1],s=40,c='g',marker='s')
#print(exitnodeid)
...
for j in range(num_user):
    if usernodelist[j] not in list_bad_path_index:
        pyplot.scatter(node_dic[usernodelist[i]][0],node_dic[usernodelist[i]][1],s=100,c=colors[0],marker='*',label='Electric Vehicle')
        tot_cost_value=0
        for i in range(len(path_list[j])-1):
            pyplot.plot([node_dic[path_list[j][i][0]][0],node_dic[path_list[j][i+1][0]][0]],[node_dic[path_list[j][i][0]][1],node_dic[path_list[j][i+1][0]][1]],lw=1.2,c=colors[0])
            tot_cost_value=tot_cost_value+edge_dic_r[(path_list[j][i][0],path_list[j][i+1][0])][0]
        tot_cost.append(tot_cost_value)
    else:
        pyplot.scatter(node_dic[usernodelist[i]][0],node_dic[usernodelist[i]][1],s=100,c=colors[1],marker='*',label='Electric Vehicle')
        for i in range(len(path_list[j])-1):
            pyplot.plot([node_dic[path_list[j][i][0]][0],node_dic[path_list[j][i+1][0]][0]],[node_dic[path_list[j][i][0]][1],node_dic[path_list[j][i+1][0]][1]],lw=1.2,c=colors[1]) 
